...
- If you have 10 original attributes, you start with 10 principal components. However, you might be find that the first 3 components explain 95% all of the variance. By keeping only those 3, you reduce your 10D problem to a 3D problem while losing only a tiny fraction of the original information.
- Each principal component is perpendicular (orthogonal) to the ones before it. This means PC2 is completely uncorrelated with PC1. In such a dataset where many attributes are redundant (like "square footage" and "number of rooms"), PCA strips away that overlap, ensuring each new component provides unique information.
- Since PCA relies heavily on variance and mean-centering—both of which involve squared distance calculations—extreme outlier can disproportionately skew the directions of the principal components. A single massive outlier can pull PC1 toward itself, failing to capture the true dominant trend of the remaining dataset.
- PCA is extremely sensitive to the scale of your data. If one attribute is measured in "kilometers" and another in "millimeters," the one with the larger number will falsely appear to have higher variance. Like for a PCA to work correctly, you must standardize your data (centering it so the mean is 0 and the standard deviation is 1) before running to the algorithm.
- Behind the scenes, PCA uses linear algebra. Eigenvectors determine the direction of the new axes (the principal components), while Eigenvalues determine the magnitude or the amount of variance explained to the that specific component. A high eigenvalue means that principal components is carrying a lot of "weights."
...
- High correlation between input attributes (multicollinearity) can cause algorithms like Linear or Logistic Regression to become numerically unstable with unreliable coefficient estimates. Because all principal components are strictly orthogonal (uncorrelated), passing them into downstream models completely eliminates multicollinearity issue.
...
- Preserving Information Variance: Each principal component carries a certain percentage of the total dataset variance. By checking the explained_variance_ratio_, you can see exactly how much information was retained after the dimensionality reductions.
- Dimensionality Reduction vs. Feature Selection: Unlike feature selection, which picks a subset of original columns, PCA creates entirely new variables. These new components are linear combinations of the original attributes, designed to capture the maximum possible spread of data.
